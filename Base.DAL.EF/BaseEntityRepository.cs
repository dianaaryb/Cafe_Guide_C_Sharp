...

{
    ...
    public virtual TDalEntity Add(TDalEntity entity, TKey? userId = default)
    {
        ...
        var entry = RepoDbSet.Add(domainEntity ?? throw new InvalidOperationException());
        RepoDbContext.SaveChanges();
        return Mapper.Map(entry.Entity) ?? throw new InvalidOperationException();
    }
    ...
    public virtual TDalEntity Update(TDalEntity entity, TKey? userId = default)
    {
        ...
        var entry = RepoDbSet.Update(domainEntity ?? throw new InvalidOperationException());
        RepoDbContext.SaveChanges();
        return Mapper.Map(entry.Entity) ?? throw new InvalidOperationException();
    }
    ...
    //     {
    //         // Handle the deletion for InMemory database since ExecuteDelete doesn't work
    //         var entityToRemove = userId == null
    //             ? RepoDbSet.FirstOrDefault(e => e.Id.Equals(entity.Id))
    //             : CreateQuery(userId).FirstOrDefault(e => e.Id.Equals(entity.Id));
    //
    ...
    //         }
    //         return 0; // No entity found to remove
    //     }
    //     else
    //     {
    //         if (userId == null)
